...
export function StudentsManager() {
  ...
  useEffect(() => {
    ...
    void loadStudents(debouncedSearch, currentPage);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await loadStudents(debouncedSearch, currentPage);
    } catch (error) {
      const message =
  ...
  async function handleToggleStudent(row: StudentItem) {
    const response = await fetch("/api/students", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  async function handleResetPassword(row: StudentItem) {
    const response = await fetch("/api/students", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        mobileCardRender={(row) => (
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              Total Classes: {row._count?.classes ?? 0}
            </p>
            <div className="mt-2">
              <span
